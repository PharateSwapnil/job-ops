#!/usr/bin/env python3
"""
Job-Ops Setup Script
====================
One-file installer and launcher for Job-Ops (frontend + backend).
Works on Windows, macOS, and Linux with Python 3.8+.

Usage
-----
    python setup.py              # Interactive guided setup
    python setup.py --install    # Install dependencies only
    python setup.py --dev        # Start in development mode (hot-reload)
    python setup.py --prod       # Build client + start production server
    python setup.py --docker     # Build & run via Docker Compose
    python setup.py --reset-db   # Wipe and re-run DB migrations
    python setup.py --check      # Check prerequisites without installing
    python setup.py --update     # Pull latest code + reinstall deps
    python setup.py --status     # Show installation status

Requirements: Python 3.8+, Node 20+, npm 9+
"""

from __future__ import annotations

import argparse
import os
import platform
import re
import shutil
import signal
import subprocess
import sys
import textwrap
from pathlib import Path
from typing import List, Optional, Tuple

# ─── Platform detection ───────────────────────────────────────────────────────

IS_WINDOWS = platform.system() == "Windows"
IS_MACOS   = platform.system() == "Darwin"
IS_LINUX   = platform.system() == "Linux"

# ─── Constants ────────────────────────────────────────────────────────────────

REPO_ROOT    = Path(__file__).parent.resolve()
ORCHESTRATOR = REPO_ROOT / "orchestrator"
SHARED       = REPO_ROOT / "shared"
DATA_DIR     = REPO_ROOT / "data"
ENV_FILE     = REPO_ROOT / ".env"
ENV_EXAMPLE  = REPO_ROOT / ".env.example"

MIN_NODE_MAJOR = 20
MIN_NPM_MAJOR  = 9
DEFAULT_PORT   = 3005

BANNER = r"""
     _       _            ___
    | | ___ | |__        / _ \ _ __  ___
 _  | |/ _ \| '_ \ ___ | | | | '_ \/ __|
| |_| | (_) | |_) |___|| |_| | |_) \__ \
 \___/ \___/|_.__/       \___/| .__/|___/
                               |_|
"""

# ─── ANSI colours ─────────────────────────────────────────────────────────────

_USE_COLOR = sys.stdout.isatty() and not IS_WINDOWS


def _c(code: str, text: str) -> str:
    return f"\033[{code}m{text}\033[0m" if _USE_COLOR else text


def green(t: str)  -> str: return _c("32", t)
def yellow(t: str) -> str: return _c("33", t)
def red(t: str)    -> str: return _c("31", t)
def cyan(t: str)   -> str: return _c("36", t)
def bold(t: str)   -> str: return _c("1",  t)
def dim(t: str)    -> str: return _c("2",  t)


# ─── Logging helpers ──────────────────────────────────────────────────────────

def info(msg: str) -> None: print(f"  {cyan('->') if not _USE_COLOR else cyan('→')} {msg}")
def ok(msg: str)   -> None: print(f"  {green('[OK]') if not _USE_COLOR else green('✓')} {msg}")
def warn(msg: str) -> None: print(f"  {yellow('[!!]') if not _USE_COLOR else yellow('⚠')} {msg}")
def err(msg: str)  -> None: print(f"  {red('[ERR]') if not _USE_COLOR else red('✗')} {msg}", file=sys.stderr)


def step(title: str) -> None:
    print(f"\n{bold(title)}")
    print(dim("-" * 50))


def abort(msg: str, code: int = 1) -> None:
    err(msg)
    sys.exit(code)


# ─── Windows command resolution ───────────────────────────────────────────────

def _which(name: str) -> Optional[str]:
    """
    Cross-platform shutil.which that also checks .cmd/.bat on Windows.
    Returns the full path string, or None if not found.
    """
    found = shutil.which(name)
    if found:
        return found
    if IS_WINDOWS:
        for ext in (".cmd", ".bat", ".exe", ".ps1"):
            found = shutil.which(name + ext)
            if found:
                return found
    return None


def _cmd(name: str) -> str:
    """
    Return the right executable name for the current OS.
    On Windows: npm → npm.cmd, npx → npx.cmd, etc.
    """
    if not IS_WINDOWS:
        return name
    # Check if the .cmd variant exists anywhere in PATH
    if shutil.which(name + ".cmd"):
        return name + ".cmd"
    if shutil.which(name + ".bat"):
        return name + ".bat"
    # Fallback — let the OS sort it out (will fail with a clear message)
    return name


# Resolved once at import time
NPM = _cmd("npm")
NPX = _cmd("npx")
NODE = _cmd("node")
GIT = _cmd("git")


# ─── Shell helpers ────────────────────────────────────────────────────────────

def _build_env(extra: Optional[dict] = None) -> dict:
    return {**os.environ, **(extra or {})}


def run(
    cmd: List[str],
    cwd: Optional[Path] = None,
    env: Optional[dict] = None,
    capture: bool = False,
    check: bool = True,
) -> subprocess.CompletedProcess:
    """Run a command list, streaming output unless capture=True."""
    kwargs: dict = dict(
        cwd=str(cwd or REPO_ROOT),
        env=_build_env(env),
        text=True,
        encoding="utf-8",
        errors="replace",
    )
    if capture:
        kwargs["stdout"] = subprocess.PIPE
        kwargs["stderr"] = subprocess.PIPE
    try:
        return subprocess.run(cmd, check=check, **kwargs)
    except subprocess.CalledProcessError as exc:
        if check:
            abort(f"Command failed (exit {exc.returncode}): {' '.join(cmd)}")
        return exc  # type: ignore[return-value]
    except FileNotFoundError:
        abort(f"Executable not found: {cmd[0]}")
    except Exception as exc:
        abort(f"Unexpected error running {cmd[0]}: {exc}")


def get_output(cmd: List[str], cwd: Optional[Path] = None) -> str:
    """Return stdout of a command, stripped. Returns '' on any error."""
    try:
        result = subprocess.run(
            cmd,
            cwd=str(cwd or REPO_ROOT),
            stdout=subprocess.PIPE,
            stderr=subprocess.DEVNULL,
            text=True,
            encoding="utf-8",
            errors="replace",
            timeout=15,
        )
        return result.stdout.strip()
    except Exception:
        return ""


def get_output_shell(cmd: str, cwd: Optional[Path] = None) -> str:
    """Run a shell string and return stdout. Returns '' on any error."""
    try:
        result = subprocess.run(
            cmd,
            cwd=str(cwd or REPO_ROOT),
            shell=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.DEVNULL,
            text=True,
            encoding="utf-8",
            errors="replace",
            timeout=15,
        )
        return result.stdout.strip()
    except Exception:
        return ""


# ─── Prerequisite checks ──────────────────────────────────────────────────────

def parse_semver(version_str: str) -> Tuple[int, int, int]:
    """Parse 'v18.19.0' or '18.19.0' or 'npm 9.8.1' → (major, minor, patch)."""
    parts = re.findall(r"\d+", version_str)
    major = int(parts[0]) if len(parts) > 0 else 0
    minor = int(parts[1]) if len(parts) > 1 else 0
    patch = int(parts[2]) if len(parts) > 2 else 0
    return major, minor, patch


def check_node() -> bool:
    ver = get_output([NODE, "--version"])
    if not ver:
        # Second attempt: shell fallback (helps on some Windows setups)
        ver = get_output_shell("node --version")
    if not ver:
        err("Node.js not found. Install from https://nodejs.org (v20+)")
        return False
    major, _, _ = parse_semver(ver)
    if major < MIN_NODE_MAJOR:
        err(f"Node.js {ver} found — need v{MIN_NODE_MAJOR}+. Install from https://nodejs.org")
        return False
    ok(f"Node.js {ver}")
    return True


def check_npm() -> bool:
    # Try multiple strategies — Windows PATH resolution is messy
    ver = ""
    for candidate in [NPM, "npm.cmd", "npm.bat", "npm"]:
        ver = get_output([candidate, "--version"])
        if ver:
            break
    if not ver:
        # Last resort: shell invocation
        ver = get_output_shell("npm --version")
    if not ver:
        err(
            "npm not found on PATH.\n"
            "    It ships with Node.js. On Windows, try:\n"
            "    1. Close and reopen PowerShell/CMD after installing Node.js\n"
            "    2. Or run: corepack enable"
        )
        return False
    major, _, _ = parse_semver(ver)
    if major < MIN_NPM_MAJOR:
        err(
            f"npm v{ver} found — need v{MIN_NPM_MAJOR}+.\n"
            f"    Run: npm install -g npm@latest"
        )
        return False
    ok(f"npm v{ver}")
    return True


def check_git() -> bool:
    ver = get_output([GIT, "--version"])
    if not ver:
        ver = get_output_shell("git --version")
    if not ver:
        warn("git not found — update feature won't work")
        return False
    ok(f"{ver}")
    return True


def check_docker() -> bool:
    docker_path = _which("docker")
    if not docker_path:
        warn("Docker not found — docker mode unavailable")
        return False
    ver = get_output([docker_path, "--version"])
    if not ver:
        warn("Docker not responding — docker mode unavailable")
        return False

    # docker compose (v2 plugin) preferred; fall back to docker-compose (v1)
    compose = get_output([docker_path, "compose", "version"])
    if not compose:
        dc = _which("docker-compose")
        if dc:
            compose = get_output([dc, "--version"])
    if not compose:
        warn("Docker Compose not found — docker mode unavailable")
        return False

    ok(f"Docker: {ver.split(',')[0]}  Compose: {compose.split(',')[0]}")
    return True


def check_playwright() -> bool:
    """Check if Playwright is installed (node_modules present)."""
    pw_path = ORCHESTRATOR / "node_modules" / "playwright"
    return pw_path.exists()


def run_checks(verbose: bool = True) -> dict:
    if verbose:
        step("Checking prerequisites")
    return {
        "node": check_node(),
        "npm":  check_npm(),
        "git":  check_git(),
        "docker": check_docker(),
    }


# ─── .env scaffolding ─────────────────────────────────────────────────────────

ENV_TEMPLATE = """\
# ──────────────────────────────────────────────────────────────────────────────
# Job-Ops Environment Configuration
# Generated by setup.py — edit as needed.
# ──────────────────────────────────────────────────────────────────────────────

# Server
PORT=3005
NODE_ENV=development
DATA_DIR=./data

# LLM provider (required for ATS scoring, tailoring, AI Q&A)
# Supported: openrouter | anthropic | openai | ollama | lmstudio | gemini
LLM_PROVIDER=openrouter
LLM_API_KEY=your_api_key_here
LLM_MODEL=google/gemini-flash-1.5

# Authentication — change these in production
JOBOPS_AUTH_SECRET=change_me_super_secret_jwt_key

# Browser automation
# Secret for AES-256-GCM credential encryption (min 32 chars)
AUTOMATION_SECRET=change_me_automation_secret_32chars_min
# Set to "false" to show the browser window during automation (debugging)
AUTOMATION_HEADLESS=true
"""


def scaffold_env() -> None:
    step("Environment configuration (.env)")
    if ENV_FILE.exists():
        ok(f".env already exists at {ENV_FILE}")
        return
    if ENV_EXAMPLE.exists():
        shutil.copy(ENV_EXAMPLE, ENV_FILE)
        ok("Copied .env.example -> .env")
    else:
        ENV_FILE.write_text(ENV_TEMPLATE, encoding="utf-8")
        ok("Created .env from template")
    warn("Edit .env and set LLM_API_KEY before starting the server.")
    warn("Also change JOBOPS_AUTH_SECRET and AUTOMATION_SECRET for production.")


# ─── Data directory ───────────────────────────────────────────────────────────

def ensure_data_dir() -> None:
    for sub in ["pdfs", "resumes", "automation/profiles", "automation/screenshots"]:
        (DATA_DIR / sub).mkdir(parents=True, exist_ok=True)
    ok(f"Data directory ready: {DATA_DIR}")


# ─── npm helpers ──────────────────────────────────────────────────────────────

def _npm(*args: str, cwd: Optional[Path] = None, env: Optional[dict] = None) -> None:
    """Run npm with the correct executable for the current OS."""
    run([NPM, *args], cwd=cwd or REPO_ROOT, env=env)


def _npx(*args: str, cwd: Optional[Path] = None, env: Optional[dict] = None) -> None:
    """Run npx with the correct executable for the current OS."""
    run([NPX, *args], cwd=cwd or REPO_ROOT, env=env)


def npm_install() -> None:
    step("Installing Node.js dependencies")
    lock = REPO_ROOT / "package-lock.json"
    if lock.exists():
        info("Running npm ci (clean install from lockfile) ...")
        _npm("ci", "--prefer-offline")
    else:
        info("Running npm install ...")
        _npm("install")
    ok("Node.js dependencies installed")


# ─── Playwright browser ───────────────────────────────────────────────────────

def install_playwright() -> None:
    step("Installing Playwright browser (Chromium)")
    info("This downloads ~150 MB of browser binaries ...")
    _npx("playwright", "install", "chromium", "--with-deps", cwd=ORCHESTRATOR)
    ok("Playwright Chromium installed")


# ─── DB migration ─────────────────────────────────────────────────────────────

def run_migrations(reset: bool = False) -> None:
    step("Database migrations")
    db_path = DATA_DIR / "jobs.db"

    if reset and db_path.exists():
        warn(f"Deleting existing database: {db_path}")
        confirm = input("  Type 'yes' to confirm reset: ").strip().lower()
        if confirm != "yes":
            info("Reset cancelled")
            return
        db_path.unlink()
        ok("Database deleted")

    ensure_data_dir()
    info("Running migrations ...")
    _npx(
        "tsx",
        "src/server/db/migrate.ts",
        cwd=ORCHESTRATOR,
        env={"DATA_DIR": str(DATA_DIR)},
    )
    ok("Database migrated successfully")


# ─── Client build ─────────────────────────────────────────────────────────────

def build_client() -> None:
    step("Building frontend (Vite)")
    info("Compiling React client ...")
    _npm("run", "build:client", cwd=ORCHESTRATOR)
    ok("Frontend built -> orchestrator/dist/")


# ─── Process launcher ─────────────────────────────────────────────────────────

def _launch(cmd: List[str], cwd: Path, env: Optional[dict] = None) -> None:
    """Launch a long-running process with graceful Ctrl+C handling."""
    merged = _build_env(env)
    info("Press Ctrl+C to stop")
    print()

    kwargs: dict = dict(cwd=str(cwd), env=merged, text=True, encoding="utf-8", errors="replace")
    if IS_WINDOWS:
        # On Windows we need CREATE_NEW_PROCESS_GROUP to forward Ctrl+C correctly
        kwargs["creationflags"] = subprocess.CREATE_NEW_PROCESS_GROUP

    proc = subprocess.Popen(cmd, **kwargs)

    def _shutdown(sig, frame):
        info("Shutting down ...")
        if IS_WINDOWS:
            proc.send_signal(signal.CTRL_BREAK_EVENT)
        else:
            proc.terminate()
        try:
            proc.wait(timeout=8)
        except subprocess.TimeoutExpired:
            proc.kill()
        sys.exit(0)

    signal.signal(signal.SIGINT,  _shutdown)
    signal.signal(signal.SIGTERM, _shutdown)
    if IS_WINDOWS:
        signal.signal(signal.SIGBREAK, _shutdown)  # type: ignore[attr-defined]

    proc.wait()


# ─── Dev mode ─────────────────────────────────────────────────────────────────

def start_dev() -> None:
    step("Starting development server")
    port = _get_port()
    info(f"Frontend + backend with hot-reload on http://localhost:{port}")
    _launch(
        [NPM, "run", "dev"],
        cwd=ORCHESTRATOR,
        env={"DATA_DIR": str(DATA_DIR)},
    )


# ─── Production mode ──────────────────────────────────────────────────────────

def start_prod() -> None:
    step("Starting production server")
    port = _get_port()

    dist = ORCHESTRATOR / "dist"
    if not dist.exists() or not any(dist.iterdir()):
        build_client()

    info(f"Server starting on http://localhost:{port}")
    _launch(
        [NPM, "run", "start"],
        cwd=ORCHESTRATOR,
        env={"NODE_ENV": "production", "DATA_DIR": str(DATA_DIR)},
    )


# ─── Docker mode ──────────────────────────────────────────────────────────────

def _docker_compose_cmd() -> List[str]:
    """Return the correct compose command list for this system."""
    docker = _which("docker")
    if docker:
        result = subprocess.run(
            [docker, "compose", "version"],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        )
        if result.returncode == 0:
            return [docker, "compose"]
    dc = _which("docker-compose")
    if dc:
        return [dc]
    return []


def start_docker() -> None:
    step("Docker Compose")

    compose = _docker_compose_cmd()
    if not compose:
        abort("Docker / Docker Compose not found. Install from https://docs.docker.com/get-docker/")

    if not (REPO_ROOT / "docker-compose.yml").exists():
        abort(f"docker-compose.yml not found at {REPO_ROOT}")

    scaffold_env()
    ensure_data_dir()

    info("Building and starting containers (first run may take several minutes) ...")
    run(compose + ["up", "--build", "-d"], cwd=REPO_ROOT)

    port = _get_port()
    ok(f"Job-Ops running at http://localhost:{port}")
    info(f"View logs:  docker compose logs -f job-ops")
    info(f"Stop:       docker compose down")


# ─── Update ───────────────────────────────────────────────────────────────────

def update() -> None:
    step("Updating Job-Ops")
    if not _which("git"):
        abort("git not found — cannot update")
    info("Pulling latest changes ...")
    run([GIT, "pull", "--ff-only"], cwd=REPO_ROOT)
    ok("Code updated")
    npm_install()
    run_migrations()
    ok("Update complete — restart the server to apply changes")


# ─── Helpers ──────────────────────────────────────────────────────────────────

def _get_port() -> int:
    """Read PORT from .env or return default."""
    if ENV_FILE.exists():
        for line in ENV_FILE.read_text(encoding="utf-8").splitlines():
            line = line.strip()
            if line.startswith("PORT="):
                try:
                    return int(line.split("=", 1)[1].strip())
                except ValueError:
                    pass
    return DEFAULT_PORT


def _env_value(key: str) -> Optional[str]:
    if not ENV_FILE.exists():
        return None
    for line in ENV_FILE.read_text(encoding="utf-8").splitlines():
        line = line.strip()
        if line.startswith(f"{key}="):
            return line.split("=", 1)[1].strip()
    return None


def _has_real_api_key() -> bool:
    val = _env_value("LLM_API_KEY") or ""
    return bool(val) and val not in ("your_api_key_here", "")


def _update_env(key: str, value: str) -> None:
    """Set or replace KEY=value in .env."""
    if not ENV_FILE.exists():
        ENV_FILE.write_text(f"{key}={value}\n", encoding="utf-8")
        return
    lines = ENV_FILE.read_text(encoding="utf-8").splitlines()
    found, new_lines = False, []
    for line in lines:
        if line.strip().startswith(f"{key}="):
            new_lines.append(f"{key}={value}")
            found = True
        else:
            new_lines.append(line)
    if not found:
        new_lines.append(f"{key}={value}")
    ENV_FILE.write_text("\n".join(new_lines) + "\n", encoding="utf-8")


# ─── Status display ───────────────────────────────────────────────────────────

def show_status() -> None:
    step("Job-Ops installation status")

    node_ver      = get_output([NODE, "--version"]) or get_output_shell("node --version")
    npm_ver       = get_output([NPM, "--version"])  or get_output_shell("npm --version")
    node_modules  = (ORCHESTRATOR / "node_modules").exists()
    db_exists     = (DATA_DIR / "jobs.db").exists()
    env_exists    = ENV_FILE.exists()
    has_key       = _has_real_api_key()
    playwright_ok = check_playwright()
    dist_exists   = (ORCHESTRATOR / "dist").exists()
    docker_ok     = bool(_which("docker"))

    rows = [
        ("Node.js",      node_ver or red("not found")),
        ("npm",          f"v{npm_ver}" if npm_ver else red("not found")),
        ("Docker",       green("available") if docker_ok else dim("not found")),
        ("node_modules", green("installed") if node_modules else yellow("missing — run: python setup.py --install")),
        ("Database",     green("exists") if db_exists else yellow("not created yet")),
        (".env",         green("found") if env_exists else yellow("missing")),
        ("LLM API key",  green("set") if has_key else yellow("not set")),
        ("Playwright",   green("installed") if playwright_ok else yellow("not installed")),
        ("Client build", green("built") if dist_exists else dim("not built (dev mode is fine)")),
    ]

    col_w = max(len(r[0]) for r in rows) + 2
    for label, value in rows:
        print(f"  {label:<{col_w}}{value}")

    print()
    info(f"App URL: http://localhost:{_get_port()}")


# ─── Interactive guided setup ─────────────────────────────────────────────────

def interactive_setup() -> None:
    print(green(BANNER))
    print(bold("Welcome to the Job-Ops setup wizard"))
    print(dim("This will install all dependencies and configure your environment.\n"))

    results = run_checks()
    if not results["node"] or not results["npm"]:
        print()
        abort(
            "Node.js 20+ and npm 9+ are required.\n"
            "  Download: https://nodejs.org\n"
            "  After installing, close and reopen your terminal, then try again."
        )

    scaffold_env()

    if not _has_real_api_key():
        print()
        warn("No LLM API key detected — ATS scoring and AI features won't work.")
        api_key = input("  Enter your LLM API key (or press Enter to skip): ").strip()
        if api_key:
            _update_env("LLM_API_KEY", api_key)
            ok("API key saved to .env")

    npm_install()

    print()
    want_pw = (input("  Install Playwright for browser automation? [Y/n]: ").strip().lower() or "y")
    if want_pw == "y":
        install_playwright()

    ensure_data_dir()
    run_migrations()

    print()
    print(bold("Setup complete! How would you like to start?"))
    print("  1) Development mode  (hot-reload, fastest feedback)")
    print("  2) Production mode   (optimised build)")
    print("  3) Docker            (containerised)")
    print("  4) Exit              (start manually later)")
    print()

    choice = input("  Enter 1-4 [1]: ").strip() or "1"

    if choice == "1":
        start_dev()
    elif choice == "2":
        start_prod()
    elif choice == "3":
        start_docker()
    else:
        port = _get_port()
        print()
        print(bold("To start manually:"))
        print(f"  Development:  python setup.py --dev")
        print(f"  Production:   python setup.py --prod")
        print(f"  Docker:       python setup.py --docker")
        print(f"  URL:          http://localhost:{port}")


# ─── CLI ──────────────────────────────────────────────────────────────────────

def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="setup.py",
        description="Job-Ops setup and launcher",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            Examples:
              python setup.py              # Interactive guided setup
              python setup.py --install    # Install deps only
              python setup.py --dev        # Dev server with hot-reload
              python setup.py --prod       # Build + production server
              python setup.py --docker     # Docker Compose
              python setup.py --reset-db   # Wipe DB and re-migrate
              python setup.py --check      # Prerequisite check only
              python setup.py --update     # git pull + reinstall + migrate
              python setup.py --status     # Show what is installed
        """),
    )
    g = p.add_mutually_exclusive_group()
    g.add_argument("--install",  action="store_true", help="Install dependencies only")
    g.add_argument("--dev",      action="store_true", help="Start development server")
    g.add_argument("--prod",     action="store_true", help="Build client + start production server")
    g.add_argument("--docker",   action="store_true", help="Build & run via Docker Compose")
    g.add_argument("--reset-db", action="store_true", help="Wipe database and re-run migrations")
    g.add_argument("--check",    action="store_true", help="Check prerequisites only")
    g.add_argument("--update",   action="store_true", help="Pull latest code + reinstall + migrate")
    g.add_argument("--status",   action="store_true", help="Show installation status")
    p.add_argument("--skip-playwright", action="store_true",
                   help="Skip Playwright browser installation")
    p.add_argument("--port", type=int, default=None,
                   help=f"Override port in .env (default: {DEFAULT_PORT})")
    return p


def _guard_requirements(results: dict) -> None:
    if not results["node"] or not results["npm"]:
        abort(
            "Node.js 20+ and npm 9+ are required.\n"
            "  Download Node.js: https://nodejs.org\n"
            "  After installing, close and reopen your terminal, then try again.\n\n"
            "  Windows tip: if npm is installed but not found, run:\n"
            "    corepack enable\n"
            "  then close and reopen your terminal."
        )


def main() -> None:
    parser = build_parser()
    args = parser.parse_args()

    if args.port:
        _update_env("PORT", str(args.port))

    if args.check:
        run_checks()
        return

    if args.status:
        show_status()
        return

    if args.reset_db:
        ensure_data_dir()
        run_migrations(reset=True)
        return

    if args.update:
        update()
        return

    if args.install:
        _guard_requirements(run_checks())
        scaffold_env()
        ensure_data_dir()
        npm_install()
        if not args.skip_playwright:
            install_playwright()
        run_migrations()
        ok("Installation complete")
        port = _get_port()
        print()
        print(bold("Next steps:"))
        print(f"  Development:  python setup.py --dev")
        print(f"  Production:   python setup.py --prod")
        print(f"  Docker:       python setup.py --docker")
        print(f"  URL:          http://localhost:{port}")
        return

    if args.dev:
        _guard_requirements(run_checks())
        if not (ORCHESTRATOR / "node_modules").exists():
            info("node_modules not found — installing first ...")
            npm_install()
        scaffold_env()
        ensure_data_dir()
        run_migrations()
        start_dev()
        return

    if args.prod:
        _guard_requirements(run_checks())
        if not (ORCHESTRATOR / "node_modules").exists():
            npm_install()
        scaffold_env()
        ensure_data_dir()
        run_migrations()
        start_prod()
        return

    if args.docker:
        scaffold_env()
        start_docker()
        return

    # No flag → interactive wizard
    interactive_setup()


if __name__ == "__main__":
    main()
