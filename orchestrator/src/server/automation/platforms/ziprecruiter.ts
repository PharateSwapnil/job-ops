...
export class ZipRecruiterAdapter extends BasePlatformAdapter {
  ...
  async login(username: string, password: string, reporter: StepReporter): Promise<void> {
    ...
    await this.page.goto("https://www.ziprecruiter.com/", { waitUntil: "domcontentloaded" });
    await this.randomDelay(1_000, 2_000);
    const loggedIn = await this.isLoggedIn(["[aria-label='Account']", ".user-nav-icon"]);
    if (loggedIn) { await reporter.log("info", "ZipRecruiter session reused"); return; }
    await reporter.report("login", 15, "Logging into ZipRecruiter");
    await this.page.goto("https://www.ziprecruiter.com/login", { waitUntil: "domcontentloaded" });
  ...
  async searchJobs(keywords: string, location: string, reporter: StepReporter): Promise<string[]> {
    ...
    await this.page.goto(`https://www.ziprecruiter.com/candidate/search?search=${q}&location=${l}`, { waitUntil: "domcontentloaded" });
    await this.randomDelay(2_000, 3_000);
    const links = await this.page.evaluate(() =>
      Array.from(document.querySelectorAll<HTMLAnchorElement>("[class*='job_result_link'], a.job-listing"))
  ...
  async extractJob(jobUrl: string, reporter: StepReporter): Promise<Partial<JobApplicationInput>> {
    ...
    const desc = await this.page.locator("[class*='job_description'], .jobDescriptionSection").first().textContent().catch(() => null);
  ...
  async navigateToApply(jobUrl: string, reporter: StepReporter): Promise<void> {
    ...
    await this.page.goto(jobUrl, { waitUntil: "domcontentloaded" });
    await this.randomDelay(1_500, 2_500);
    await this.clickElement("button:has-text('Apply Now'), a:has-text('Apply Now')");
    await this.randomDelay(1_500, 2_500);
  }
  ...
  async fillForm(input: JobApplicationInput, reporter: StepReporter): Promise<void> {
    ...
    const nextBtn = this.page.locator("button:has-text('Continue'), button:has-text('Next')").first();
  ...
  async answerQuestions(aiAnswers: Map<string, string>, reporter: StepReporter): Promise<void> { /* ZipRecruiter one-click apply rarely has questions */ }
  ...
  async verify(reporter: StepReporter): Promise<ApplicationResult> {
    await this.randomDelay(1_000, 2_000);
    const success = await this.page.locator("[class*='success'], h1:has-text('Applied'), h2:has-text('You Applied')").first().isVisible().catch(() => false);
    return success ? { success: true } : { success: false, errorMessage: "ZipRecruiter submission status unclear" };
  }
