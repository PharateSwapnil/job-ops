...
export class DiceAdapter extends BasePlatformAdapter {
  ...
  async login(
    ...
    await this.page.goto("https://www.dice.com/", {
    ...
    if (loggedIn) {
      await reporter.log("info", "Dice session reused");
      return;
    }
  ...
  async searchJobs(
    ...
    await this.page.goto(`https://www.dice.com/jobs?q=${q}&location=${l}`, {
      waitUntil: "domcontentloaded",
    });
  ...
  async extractJob(
    ...
    const desc = await this.page
      .locator("[data-cy='jobDescription']")
      .textContent()
      .catch(() => null);
  ...
  async navigateToApply(jobUrl: string, reporter: StepReporter): Promise<void> {
    ...
    await this.clickElement(
      "[data-cy='apply-button-top'], button:has-text('Easy Apply')",
    );
    await this.randomDelay(1_500, 2_500);
  }
  ...
  async answerQuestions(
    ...
    /* Dice rarely has screening questions */
  }
  ...
  async verify(reporter: StepReporter): Promise<ApplicationResult> {
    ...
    const success = await this.page
      .locator("[data-cy='success-message'], h2:has-text('Applied')")
      .first()
      .isVisible()
      .catch(() => false);
    return success
      ? { success: true }
      : { success: false, errorMessage: "Dice submission status unclear" };
  }
