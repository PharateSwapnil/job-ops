...
export class WellfoundAdapter extends BasePlatformAdapter {
  ...
  async login(username: string, password: string, reporter: StepReporter): Promise<void> {
    ...
    await this.page.goto("https://wellfound.com/", { waitUntil: "domcontentloaded" });
    await this.randomDelay(1_000, 2_000);

    const loggedIn = await this.isLoggedIn(["[data-test='nav-avatar']", ".user-avatar"]);
    if (loggedIn) { await reporter.log("info", "Wellfound session reused"); return; }

    await reporter.report("login", 15, "Logging into Wellfound");
    await this.page.goto("https://wellfound.com/login", { waitUntil: "domcontentloaded" });
  ...
  async searchJobs(keywords: string, location: string, reporter: StepReporter): Promise<string[]> {
    ...
    await this.page.goto(`https://wellfound.com/jobs?q=${q}`, { waitUntil: "domcontentloaded" });
  ...
  async extractJob(jobUrl: string, reporter: StepReporter): Promise<Partial<JobApplicationInput>> {
    ...
    const desc = await this.page.locator(".job-description, [class*='description']").first().textContent().catch(() => null);
  ...
  async fillForm(input: JobApplicationInput, reporter: StepReporter): Promise<void> {
    if (input.coverLetter) {
      await this.fillTextField("textarea[placeholder*='cover'], textarea[name*='cover']", input.coverLetter);
    }
  }

  async answerQuestions(aiAnswers: Map<string, string>, reporter: StepReporter): Promise<void> { /* No standard Q&A */ }
