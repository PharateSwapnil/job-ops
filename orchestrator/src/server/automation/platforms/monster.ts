...
export class MonsterAdapter extends BasePlatformAdapter {
  ...
  async login(username: string, password: string, reporter: StepReporter): Promise<void> {
    ...
    await this.page.goto("https://www.monster.com/", { waitUntil: "domcontentloaded" });
    await this.randomDelay(1_000, 2_000);
    const loggedIn = await this.isLoggedIn(["[data-testid='user-menu']", ".user-nav"]);
    if (loggedIn) { await reporter.log("info", "Monster session reused"); return; }
    await reporter.report("login", 15, "Logging into Monster");
    await this.page.goto("https://www.monster.com/login", { waitUntil: "domcontentloaded" });
  ...
  async searchJobs(keywords: string, location: string, reporter: StepReporter): Promise<string[]> {
    ...
    await this.page.goto(`https://www.monster.com/jobs/search?q=${q}&where=${l}`, { waitUntil: "domcontentloaded" });
    await this.randomDelay(2_000, 3_000);
    const links = await this.page.evaluate(() =>
      Array.from(document.querySelectorAll<HTMLAnchorElement>("[data-testid='jobTitle']"))
  ...
  async extractJob(jobUrl: string, reporter: StepReporter): Promise<Partial<JobApplicationInput>> {
    ...
    const desc = await this.page.locator(".job-description, [data-testid='JobDescription']").first().textContent().catch(() => null);
  ...
  async navigateToApply(jobUrl: string, reporter: StepReporter): Promise<void> {
    ...
    await this.page.goto(jobUrl, { waitUntil: "domcontentloaded" });
    await this.randomDelay(1_500, 2_500);
    await this.clickElement("[data-testid='applyButton'], a:has-text('Apply Now')");
    await this.randomDelay(1_500, 2_500);
  }
  ...
  async answerQuestions(aiAnswers: Map<string, string>, reporter: StepReporter): Promise<void> { /* Monster rarely has inline Q&A */ }

  async submit(reporter: StepReporter): Promise<void> {
    await this.clickElement("button[type='submit']:has-text('Apply'), button:has-text('Submit Application')");
    await this.randomDelay(2_000, 3_500);
  }

  async verify(reporter: StepReporter): Promise<ApplicationResult> {
    await this.randomDelay(1_000, 2_000);
    const success = await this.page.locator("h1:has-text('Application Submitted'), [class*='success']").first().isVisible().catch(() => false);
    return success ? { success: true } : { success: false, errorMessage: "Monster submission status unclear" };
  }
