...
export class WorkdayAdapter extends BasePlatformAdapter {
  ...
  async login(username: string, password: string, reporter: StepReporter): Promise<void> {
    ...
    const signInBtn = this.page.locator("button:has-text('Sign In'), a:has-text('Sign In')").first();
    if (await signInBtn.isVisible().catch(() => false)) {
      ...
      await this.fillTextField("input[data-automation-id='password']", password);
      ...
      await reporter.log("info", "Workday — no sign-in prompt; proceeding as guest or existing session");
    }
  }

  async searchJobs(_keywords: string, _location: string, reporter: StepReporter): Promise<string[]> {
    await reporter.log("info", "Workday search requires company-specific URL — use direct job URLs");
    return [];
  }
  ...
  async navigateToApply(jobUrl: string, reporter: StepReporter): Promise<void> {
    ...
    await this.withRetry(async () => {
      const btn = this.page.locator(
        "[data-automation-id='applyBtn'], button:has-text('Apply')",
      ).first();
  ...
  async fillForm(input: JobApplicationInput, reporter: StepReporter): Promise<void> {
    ...
    for (let pg = 0; pg < MAX_PAGES; pg++) {
      ...
      for (let i = 0; i < cnt; i++) {
        ...
        if (!val) {
          ...
          if (labelText && input.aiAnswers.get(labelText)) {
            await this.humanType(el, input.aiAnswers.get(labelText)!, { clear: true });
            await this.randomDelay(300, 600);
          }
        }
      }
      ...
      // Review page
      const reviewBtn = this.page.locator(
        "[data-automation-id='bottom-navigation-review-btn']",
      ).first();
  ...
  async verify(reporter: StepReporter): Promise<ApplicationResult> {
    ...
    const success = await this.page
      .locator("[data-automation-id='thankYouBanner'], h1:has-text('Thank You')")
      .first()
      .isVisible()
      .catch(() => false);
    return success
  ...
  async logout(reporter: StepReporter): Promise<void> {
    await this.clickElement("[data-automation-id='signOut'], a:has-text('Sign Out')");
