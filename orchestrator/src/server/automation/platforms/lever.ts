...
export class LeverAdapter extends BasePlatformAdapter {
  ...
  async login(
    ...
    await reporter.log(
      "info",
      "Lever requires no site login — applying directly",
    );
  }
  ...
  async searchJobs(
    ...
    await reporter.log(
      "info",
      "Lever search not supported — use direct job URLs",
    );
    return [];
  }
  ...
  async extractJob(
    ...
    const desc = await this.page
      .locator(".posting-page .section-wrapper")
      .first()
      .textContent()
      .catch(() => null);
  ...
  async submit(reporter: StepReporter): Promise<void> {
    await this.withRetry(
      async () => {
        const btn = this.page
          .locator("button[type='submit']:has-text('Submit application')")
          .first();
