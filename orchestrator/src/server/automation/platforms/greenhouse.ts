...
export class GreenhouseAdapter extends BasePlatformAdapter {
  ...
  async login(_username: string, _password: string, reporter: StepReporter): Promise<void> {
    await reporter.log("info", "Greenhouse requires no site login — proceeding anonymously");
  }

  async searchJobs(_keywords: string, _location: string, reporter: StepReporter): Promise<string[]> {
    await reporter.log("info", "Greenhouse search is not supported — provide direct job URLs");
    return [];
  }
  ...
  async extractJob(jobUrl: string, reporter: StepReporter): Promise<Partial<JobApplicationInput>> {
    ...
    const desc = await this.page.locator("#content .job-post").textContent().catch(() => null);
  ...
  async uploadResume(resumePath: string, reporter: StepReporter): Promise<void> {
    ...
    if (uploaded) await reporter.log("info", "Resume uploaded to Greenhouse form");
  }
  ...
  async answerQuestions(aiAnswers: Map<string, string>, reporter: StepReporter): Promise<void> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const answer = labelText?.trim() ? (aiAnswers.get(labelText.trim()) ?? "") : "";
      if (answer) {
  ...
  async submit(reporter: StepReporter): Promise<void> {
    await this.withRetry(async () => {
      const btn = this.page.locator("#submit_app, input[type='submit'][value*='Submit']").first();
